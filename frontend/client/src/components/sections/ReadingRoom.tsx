...
import {
  ...
  Type, Minus, Plus, AlignLeft, ArrowLeft
} from "lucide-react";
...
export function ReadingRoom() {
  ...
  const toggleBookmark = () => {
    setBookmarks(p => {
      const updated = { ...p, [selectedBook.id]: !p[selectedBook.id] };
      const activeIds = Object.keys(updated).filter(id => updated[id]);
      const token = localStorage.getItem("auth_token");
      if (token) {
        fetch("/api/user/bookmarks", {
          method: "POST",
          headers: { "Content-Type": "application/json", Authorization: `Bearer ${token}` },
          body: JSON.stringify({ bookmarks: activeIds })
        }).catch(err => console.error("Error syncing bookmarks:", err));
      }
      return updated;
    });
  };
  ...
  return (
    <div className="bg-dharma-ink min-h-screen text-dharma-ivory font-sans flex flex-col justify-between">
      ...
      <motion.section
        ...
        id="reading-room"
        className="pt-32 pb-24 bg-dharma-ink relative overflow-hidden flex-grow"
      >
        {/* Ambient background */}
        ...
        <div className="container mx-auto px-6 max-w-7xl relative z-10">
          ...
          <div className="mb-8 flex items-center justify-between">
            ...
            <span className="text-xs text-dharma-ivory-dim font-medium">Digital Sanctuary Library</span>
          </div>
          ...
          {filteredBooks.length > 0 ? filteredBooks.map((book, idx) => {
            ...
            return (
              ...
              >
                ...
                {/* Cover */}
                <div className="relative aspect-[3/4] overflow-hidden bg-dharma-ink-3">
                  <img
                    src={book.cover}
